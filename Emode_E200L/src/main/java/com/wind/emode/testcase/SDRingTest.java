...
public class SDRingTest extends BaseActivity {
	...
	// modified by lizusheng@wind-mobi.com 20151214 for bug #92854 start
	...
	// modified by lizusheng@wind-mobi.com 20151214 for bug #92854 end
	...
	protected void onCreate(Bundle icicle) {
		...
		String externalPath = getExternalPath();
		Log.d(LOG_TAG, "externalPath:" + externalPath);
		/*
		 * StorageManager mStorageManager = (StorageManager)
		 * getSystemService(Context.STORAGE_SERVICE); StorageVolume[] volumes =
		 * mStorageManager.getVolumeList(); for (StorageVolume volume : volumes)
		 * { if (volume.isRemovable() &&
		 * Environment.MEDIA_MOUNTED.equals(mStorageManager
		 * .getVolumeState(volume.getPath()))) { externalPath =
		 * volume.getPath(); break; } }
		 */
		...
		// add by lizusheng@wind-mobi.com 20151214 for bug #92854 start
		filter.addAction(Intent.ACTION_MEDIA_UNMOUNTED);
		filter.addDataScheme("file");
	//add by lizusheng@wind-mobi.com 20151214 for bug #92854 end
...
//        }
	...
	private void updateScreen() {
		...
		if (hasSdCard()) {
			...
			if(btn_pass.getVisibility()==View.VISIBLE){
				btn_pass.setVisibility(View.GONE);
			}			
		}
	}
