...
public class ImeiWrite extends Activity implements OnClickListener {
	...
	private static final boolean MTK_GEMINI_SUPPORT = "1"
			.equals(SystemProperties.get("ro.mtk_gemini_support"));
		...
		public void handleMessage(Message msg) {
			...
			case EVENT_WRITE_MEID:
				...
				if (ar.exception == null) {
					...
					mAlertDialog.setTitle("MEID WRITE");
					mAlertDialog
							.setMessage("Fail to write MEID due to radio unavailable or something else.");
					mAlertDialog.show();
				}
			default:
				break;
	...
	private void writeMeid(String meid) {
		String imeiString[] = { "at+vmobid=1,", "" };
		imeiString[0] = "at+auso=9";
		imeiString[1] = "at+vmobid=1,\"7268324842763108\",2," + "\"" + meid
				+ "\"";
	...
	public void onClick(View arg0) {
		...
		if (arg0.getId() == mBtnMeid.getId()) {
			if ("".equals(imei)) {
				Toast.makeText(this, "MEID must not be null!",
						Toast.LENGTH_LONG).show();
				return;
			}/*
			 * else if(imei.length() != 14){ Toast.makeText(this,
			 * "MEID length must be 15!", Toast.LENGTH_LONG).show(); return; }
			 */
		} else if (arg0.getId() == mBtnSim1.getId()
				|| arg0.getId() == mBtnSim2.getId()) {
			if ("".equals(imei)) {
				Toast.makeText(this, "IMEI must not be null!",
						Toast.LENGTH_LONG).show();
				return;
			} else if (imei.length() != 15) {
				Toast.makeText(this, "IMEI length must be 15!",
						Toast.LENGTH_LONG).show();
				return;
			} else {
				...
				if (cb != -1) {
					Toast.makeText(this,
							"IMEI error! end number may be " + cb + " !",
							Toast.LENGTH_LONG).show();
					return;
				}
			}
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		String imei1 = TelephonyManagerEx.getDefault().getDeviceId(
				PhoneConstants.SIM_ID_1);// PhoneFactory.getPhone(PhoneConstants.SIM_ID_1).getDeviceId();
		// String imei1 = SystemProperties.get("ro.wind_imei");
		Log.d("lizusheng", "MEID1:" + imei1);
		if (imei1 != null && !"".equals(imei1)) {
			mBtnSim1.setEnabled(false);
			((TextView) findViewById(R.id.txt_imei1)).setText("1. IMEI1: "
					+ imei1);
		}
		if (!MTK_GEMINI_SUPPORT) {
			mBtnSim2.setEnabled(false);
		} else {
			String imei2 = TelephonyManagerEx.getDefault().getDeviceId(
					PhoneConstants.SIM_ID_2);// PhoneFactory.getPhone(PhoneConstants.SIM_ID_2).getDeviceId();
			// String imei2 = SystemProperties.get("ro.wind_imei2");
			Log.d("lizusheng", "MEID2:" + imei2);
			if (imei2 != null && !"".equals(imei2)) {
				mBtnSim2.setEnabled(false);
				((TextView) findViewById(R.id.txt_imei2)).setText("2. IMEI2: "
						+ imei2);
			}
		}
		// String meid = SystemProperties.get("ro.wind_meid");
		/*
		 * Log.d("lizusheng", "MEID:"+meid); if(null!=meid && !"".equals(meid)&&
		 * !"00000000000000".equals(meid)){ mBtnMeid.setEnabled(false);
