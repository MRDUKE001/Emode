...
import com.mediatek.fmradio.FmRadioListener;
...
public class FMTest extends BaseActivity {
    ...
    private Button mButtonFreqSub = null;
    private SeekBar mSbFreq = null;
    private Button mButtonFreqAdd = null;
    private Button mButtonFreq1 = null;
    private Button mButtonFreq2 = null;
    private Button mButtonFreq3 = null;
    private Button mButtonFreq4 = null;
	...
	private boolean[] mPassTypes = {false, false,false,false,false,false};
         ...
         public void onReceive(Context context, Intent intent) {    
            if (intent.getIntExtra("state", 0) == 0) { // unpluged
                ...
                if (mIsPlaying) {
                    ...
                    float freq = FmRadioUtils.computeFrequency(mCurrentStation);
                    mSbFreq.setProgress((int)((freq - 80)*10));
                    tuneToStation(mCurrentStation);
                    mEditFreq.setText(getString(R.string.FMR_FREQ_TIP)+" " + freq + " MHz");
                }
...
//                }
            ...
            case R.id.FMR_Freq_tune:
                ...
                break;*/
            case R.id.FMR_Freq_sub:
                mSbFreq.setProgress(mSbFreq.getProgress() - 1);
                float freq_sub = 80 + mSbFreq.getProgress() / 10.0f;
                tuneToStation(FmRadioUtils.computeStation(freq_sub));
                mEditFreq.setText(getString(R.string.FMR_FREQ_TIP)+" " + freq_sub +" MHz");
                testPass(0);
                break;
            case R.id.FMR_Freq_add:
                mSbFreq.setProgress(mSbFreq.getProgress() + 1);
                float freq_add = 80 + mSbFreq.getProgress() / 10.0f;
                tuneToStation(FmRadioUtils.computeStation(freq_add));
                mEditFreq.setText(getString(R.string.FMR_FREQ_TIP)+" " + freq_add + " MHz");
                testPass(1);
                break;
            case R.id.FMR_Freq_1:
               mSbFreq.setProgress(90);
                tuneToStation(FmRadioUtils.computeStation(89));
                mEditFreq.setText(getString(R.string.FMR_FREQ_TIP)+" 89 MHz");
                testPass(2);
                break;
            case R.id.FMR_Freq_2:
                mSbFreq.setProgress(93);
                tuneToStation(FmRadioUtils.computeStation(89.3f));
                mEditFreq.setText(getString(R.string.FMR_FREQ_TIP)+" 89.3 MHz");
                testPass(3);
                break;
            case R.id.FMR_Freq_3:
                mSbFreq.setProgress(190);
                tuneToStation(FmRadioUtils.computeStation(99));
                mEditFreq.setText(getString(R.string.FMR_FREQ_TIP)+" 99 MHz");
                testPass(4);
                break;
            case R.id.FMR_Freq_4:
                mSbFreq.setProgress(220);
                tuneToStation(FmRadioUtils.computeStation(102));
                mEditFreq.setText(getString(R.string.FMR_FREQ_TIP)+" 102 MHz");
                testPass(5);
                break;
            default :
    ...
    private SeekBar.OnSeekBarChangeListener mSeekbarListener = new SeekBar.OnSeekBarChangeListener() {
        ...
        public void onProgressChanged(SeekBar seekBar, int progress, boolean fromUser){
            mEditFreq.setText(getString(R.string.FMR_FREQ_TIP)+" "+(80+progress/10.0f)+"MHz");
        }
        ...
        public void onStopTrackingTouch(SeekBar seekBar){
            float freq = 80 + mSbFreq.getProgress() / 10.0f;
            tuneToStation(FmRadioUtils.computeStation(freq));
            mEditFreq.setText(getString(R.string.FMR_FREQ_TIP)+" "+freq+" MHz");
        }
    };
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        mEditFreq = (TextView) findViewById(R.id.FMR_Freq_tip);
        mButtonFreqSub = (Button) findViewById(R.id.FMR_Freq_sub);
        mSbFreq = (SeekBar) findViewById(R.id.FMR_Freq_seek);
        mButtonFreqAdd = (Button) findViewById(R.id.FMR_Freq_add);
        mButtonFreq1 = (Button) findViewById(R.id.FMR_Freq_1);
        mButtonFreq2 = (Button) findViewById(R.id.FMR_Freq_2);
        mButtonFreq3 = (Button) findViewById(R.id.FMR_Freq_3);
        mButtonFreq4 = (Button) findViewById(R.id.FMR_Freq_4);
        mButtonFreqSub.setOnClickListener(mButtonClickListener);
        mSbFreq.setOnSeekBarChangeListener(mSeekbarListener);
        mButtonFreqAdd.setOnClickListener(mButtonClickListener);
        mButtonFreq1.setOnClickListener(mButtonClickListener);
        mButtonFreq2.setOnClickListener(mButtonClickListener);
        mButtonFreq3.setOnClickListener(mButtonClickListener);
        mButtonFreq4.setOnClickListener(mButtonClickListener);
        mButtonFreqSub.setText("-");
        mButtonFreqAdd.setText("+");
        mButtonFreq1.setText("89MHz");
        mButtonFreq2.setText("89.3MHz");
        mButtonFreq3.setText("99MHz");
        mButtonFreq4.setText("102MHz");
...
//        }
        ...
        mCurrentStation = FmRadioUtils.DEFAULT_STATION;//FMRadioStation.getCurrentStation(mContext);
...
//                FmRadioStation.setCurrentStation(mContext, mCurrentStation);
    ...
    protected void onStart() {
        super.onStart();
        mIsServiceBinded = bindService(new Intent(FMTest.this, FmRadioService.class), mServiceConnection,
                Context.BIND_AUTO_CREATE);
//        }
        if (!mIsServiceBinded) {
            Log.e(TAG, "Error: Cannot bind FM service");
...
//            powerDown();
    ...
    private void refreshTextStatus(boolean on) {
        if (!on) {
            ...
            //mButtonTune.setEnabled(false);
            mButtonFreqSub.setEnabled(false);
            mSbFreq.setEnabled(false);
            mButtonFreqAdd.setEnabled(false);
            mButtonFreq1.setEnabled(false);
            mButtonFreq2.setEnabled(false);
            mButtonFreq3.setEnabled(false);
            mButtonFreq4.setEnabled(false);
            mEditFreq.setText(getString(R.string.plug_headset_tip));
            mEditFreq.setTextColor(Color.RED);
        } else {
            //mButtonTune.setEnabled(true);
            mButtonFreqSub.setEnabled(true);
            mSbFreq.setEnabled(true);
            mButtonFreqAdd.setEnabled(true);
            mButtonFreq1.setEnabled(true);
            mButtonFreq2.setEnabled(true);
            mButtonFreq3.setEnabled(true);
            mButtonFreq4.setEnabled(true);
            if(getString(R.string.plug_headset_tip).equals(mEditFreq.getText())){
                mEditFreq.setText("");
            }
            mEditFreq.setTextColor(Color.WHITE);
        }
    }
...
//        return false;
    ...
    public void testPass(int index){
        ...
        if(mPassTypes[0] || mPassTypes[1]|| mPassTypes[2]|| mPassTypes[3]|| mPassTypes[4]|| mPassTypes[5]){
